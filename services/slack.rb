...
class Service::Slack < Service
  def receive_logs
    ...
    message = %{"#{payload[:saved_search][:name]}" search found #{pluralize('match', :count => payload[:events].length)} — <#{payload[:saved_search][:html_search_url]}|#{payload[:saved_search][:html_search_url]}>}
